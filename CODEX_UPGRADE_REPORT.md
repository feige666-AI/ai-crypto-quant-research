...
项目现在是可安装的 Python 3.10+ `src` 包，包含四个 CLI 子命令、严格 OHLCV 校验、均线信号、教学型回测、稳定报告、合成数据生成、三张图表、77 个测试、Ruff 和 GitHub Actions。样本数据生成核心已位于正式包，顶层脚本只是跨平台命令入口。
...
- `src/crypto_quant_research/sample_data.py`
...
| `pytest -q` | CI 修复后 77 passed in 2.39s。 |
| `pytest --cov=crypto_quant_research --cov-branch --cov-report=term-missing -q` | CI 修复后 77 passed in 4.34s，总覆盖率 97%。 |
...
GitHub Actions 首次运行在 Python 3.10、3.11、3.12 均因测试导入顶层 `scripts` 失败。根因已通过把生成器移入正式包并修改测试导入路径修复；本报告不提前把尚未完成的当前远端检查写成通过，最终状态以 PR #1 当前提交为准。
...
- 编写 77 个 pytest 测试并达到 97% 核心包覆盖率，配置 Ruff 和 Python 3.10–3.12 GitHub Actions。
